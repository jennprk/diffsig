...
#' @return Diffsig plot
#'
diffsig_plot <- function(fit, pars, rowlabels, rowgroup=NULL, est_color=NULL, font=NULL) {
  ...
  p.point <- ggplot2::geom_point(ggplot2::aes_string(x = "m", y = "y", color = color_by), fill = est_color, size = 2,  shape = 21)

  p.all <- p.all +
    p.ci.2 +
    scale_color_viridis(discrete = TRUE, begin = 0.8, end=0.2,  alpha=0.7, option="A", name="Risk Factors") +
